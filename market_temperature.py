...
THRESHOLDS = {
    "PE": {"undervalued": (0, 12), "fair": (12, 16), "overvalued": (16, 999)},
    "PB": {"undervalued": (0, 1.3), "fair": (1.3, 1.8), "overvalued": (1.8, 999)},
}
...
def get_index_pe_pb(secid):
    ...
    try:
        ...
        params = {"secid": secid, "fields": "f43,f44,f115,f117,f119,f120,f121"}
...
def temp_label(pct):
    if pct is None:
        return "?", "—"
    if pct <= 30:
        return "🧊 低估", f"分位{pct}% → 可出手"
    elif pct <= 70:
        return "🌤 正常", f"分位{pct}% → 耐心等"
    else:
        return "🔥 高估", f"分位{pct}% → 收紧"
...
def main():
    ...
    for name, secid in INDICES.items():
        ...
        # 避免同日重复
        if hist["pe"] and hist["pe"][-1].startswith(today_str):
            pass
        else:
        ...
        # 整体评分
        avg_pct = (pe_pct or 50 + pb_pct or 50) / 2 if pe_pct and pb_pct else (
            pe_pct or pb_pct or 50)
    ...
    lines.append("**框架联动建议：**")
    has_undervalued = any(
        (float(v.split(":")[1]) if ":" in v else 0) < 30
        for name in INDICES
        for v in history.get(name, {}).get("pe", [])[:1]
    ) if history else False

    lines.append("> 估值偏低时触发价可适当放宽；估值偏高时严守触发价不追高。")
    lines.append("")
    lines.append("📊 累积历史{:.0f}天 | 下次更新：明天".format(
        max((len(history[n]["pe"]) for n in history if "pe" in history[n]), default=0)))
